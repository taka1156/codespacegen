...
func TestResolveBaseImage_EmptyLanguageReturnsDefaultImage(t *testing.T) {
	image, install, timezone, extensions, err := resolveBaseImage("", "", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if image != entity.DefaultImage {
		t.Fatalf("expected %s but got: %s", entity.DefaultImage, image)
	}
	if install != "" || timezone != "" {
...
func TestResolveBaseImage_UnsupportedLanguageReturnsError(t *testing.T) {
	...
	_, _, _, _, err := resolveBaseImage("unknownlang", "", cfgPath)
	if err == nil {
...
func TestResolveBaseImage_ReturnsErrorWhenImageIsMissing(t *testing.T) {
	...
	_, _, _, _, err := resolveBaseImage("go", "", cfgPath)
	if err == nil {
