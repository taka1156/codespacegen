...
| `-language` | *(対話入力、Enter で空)* | 言語キー。毎回確認される。組み込み (`go/python/node:biome/node:eslint/node:react/rust`) に加えて、`codespacegen.json` で定義した任意キーも利用可能。空の場合は言語固有設定を使わず `alpine:latest` を採用 |
...
| `-image-config` | `codespacegen.json` | ベースイメージ定義のローカルパスまたは `https://` URL。トップレベル `common` と言語別設定をサポート。`install` を指定する場合は `image` が必須。タイムゾーンや拡張機能のみのエントリでは `common` から `image` を継承可能 |
...
**形式 2: オブジェクトで install コマンド・timezone・VS Code 拡張を指定（`install` を指定する場合は `image` が必须）**
