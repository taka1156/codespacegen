...
if [ "${ARCHIVE}" = "exe" ]; then
	...
	go build -trimpath -ldflags="-s -w -X github.com/taka1156/codespacegen/internal/app.Version=${VERSION}" -o "${out}" ./cmd/codespacegen
else
	bin="tmp/codespacegen"
	go build -trimpath -ldflags="-s -w -X github.com/taka1156/codespacegen/internal/app.Version=${VERSION}" -o "${bin}" ./cmd/codespacegen
