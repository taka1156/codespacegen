...
func (ci *ClientInput) GetInput(args []string) entity.ClientConfig {
	...
	clientConfig.OutputDir = fs.String(
		"output",
		".devcontainer",
		"output directory for generated files",
	)
	clientConfig.ContainerName = fs.String(
		"name",
		"",
		"project name (required, mapped to devcontainer name)",
	)
	clientConfig.ServiceName = fs.String(
		"service",
		"",
		"docker compose service name",
	)
	clientConfig.Language = fs.String(
		"language",
		"",
		"programming language (go/python/node/rust or image-config keys)",
	)
	clientConfig.WorkspaceFolder = fs.String(
		"workspace-folder",
		"/workspace",
		"workspace folder inside container",
	)
	clientConfig.Timezone = fs.String(
		"timezone",
		"",
		"timezone inside container (default: image-config timezone or UTC)",
	)
	clientConfig.ImageConfig = fs.String(
		"image-config",
		"codespacegen.json",
		"local path or https:// URL to base image config JSON",
	)
	clientConfig.Port = fs.String(
		"port",
		"",
		"port mapping (e.g. 3000 or 3000:3000)",
	)
	clientConfig.ComposeFile = fs.String(
		"compose-file",
		"docker-compose.yaml",
		"docker compose file name",
	)
	clientConfig.EnableOverwriteFile = fs.Bool(
		"force",
		false,
		"overwrite existing files",
	)
	clientConfig.Lang = fs.String(
		"lang",
		"",
		"language for CLI messages (en/ja, default: auto-detect)",
	)
	clientConfig.Headless = fs.Bool(
		"headless",
		false,
